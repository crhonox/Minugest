...
public class Receta2Controller 
{
    ...
    public ModelAndView editarReceta (HttpServletRequest request)
    {
        ...
        mav.addObject("receta",new Receta(idReceta, datos.getNombreReceta(), datos.getIdCategoria(), datos.getDescripcionReceta()));
        return mav;
    }
        ...
        )
    {
        ...
        else
        {
            ...
            this.jdbcTemplate.update( //REVISAR ORDEN UPDATE 
                        "update RECETA set  CODIGO_CATEGORIA=?, NOMBRE_RECETA=?, DESCRIPCION_RECETA=? where CODIGO_RECETA=? ",
        rec.getIdCategoria(),  rec.getNombreReceta(), rec.getDescripcionReceta(), idReceta);
